...
{
    public class EstadosUsuarioCrudFactory : CrudFactory
    {

        private EstadosUsuarioMapper mapper;

        public EstadosUsuarioCrudFactory()
        {
            mapper = new EstadosUsuarioMapper();
        ...
        public List<T> RetrieveAllCambio<T>()
        {
            var res = new List<T>();
            var sqlOperation = mapper.GetRetriveAllCambioStatement();
            var lstResult = dao.ExecuteQueryProcedure(sqlOperation);
            if (lstResult.Count > 0)
            {
                var objs = mapper.BuildObjects(lstResult);
