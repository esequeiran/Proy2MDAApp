function vListarSolicitudesTrabajoIngresadasPorOferente() {
    ...
    this.serviceOferta = 'OfertaASolicitudDeTrabajo';
    this.serviceDetalle = 'SolicitudDeTrabajo?idSolDetalles=';
    ...
    this.Ofertar = function () {
        ...
        var validar = this.ctrlActions.ValidateDataForm('frmOfertar');
        if (validar) {
                     
     
            ofertaData = this.ctrlActions.GetDataForm('frmOfertar');

            var valorPresupuesto = ofertaData['PresupuestoOferta'];

            if (valorPresupuesto <= 0 && $('#txtPresupuestoOferta').attr("IsRequired") === "required") {
                this.ctrlActions.ShowMessage('E', 'Revise que el presupuesto sea mayor a $0');
            }
            else {
                var idSol = $('#txtIdSolicitud').val();
                ofertaData['IdSolicitud'] = idSol;
                ofertaData['IdUsuario'] = this.cedulaOferente;
                if (ofertaData['PresupuestoOferta'] == "") {
                    ofertaData['PresupuestoOferta'] = "0";
                }
                console.log(ofertaData);
                debugger
                this.ctrlActions.PostToAPI(this.serviceOferta, ofertaData);
...
$(document).ready(function () {
    ...
    var vlistarSolictiudes = new vListarSolicitudesTrabajoIngresadasPorOferente();
