class vListarLocalizaciones {
    constructor() {
        ...
        this.btnModificar = document.getElementById('btnUpdate');
        this.btnEliminar = document.getElementById('btnDelete');
        this.btnCancelar = document.getElementById('btnCancel');
        //Tabla
        this.tabla = document.getElementById('tblLocalizaciones');
        //servicios
        this.service = 'Localizacion?identificacion='+sessionStorage.getItem('IdUsuario');
        this.ctrlActions = new ControlActions();
        //métodos
        this.fillsltProvincia();
        this.localizacionesRegistradas;
        this.getLocalizaciones();
        this.actualLocalizacion;
    ...
    getLocalizaciones() {
        this.ctrlActions.GetToApi(this.service, (response) => {
            this.localizacionesRegistradas = response.Data;
            this.llenarTabla();
        });
    }
    //END
    ...
    //llenar la tabla
    llenarTabla() {
        for (let i = 0; i < this.localizacionesRegistradas.length; i++) {
            const localizacion = this.localizacionesRegistradas[i];
            this.crearColumna(localizacion, (i + 1));
        }
        this.cargarDataTable();
    }

    crearColumna(localizacion, index) {
        const tableRef = this.tabla.getElementsByTagName('tbody')[0];
        const row = tableRef.insertRow();

        const nombre = row.insertCell(0);
        const provincia = row.insertCell(1);
        const canton = row.insertCell(2);
        const distrito = row.insertCell(3);
        const otrasSennas = row.insertCell(4);
        const latitud = row.insertCell(5);
        const longitud = row.insertCell(6);

        nombre.innerHTML = localizacion.Nombre;
        provincia.innerHTML = localizacion.Provincia;
        canton.innerHTML = localizacion.Canton;
        distrito.innerHTML = localizacion.Distrito;
        otrasSennas.innerHTML = localizacion.OtrasSennas;
        latitud.innerHTML = localizacion.Latitud;
        longitud.innerHTML = localizacion.Longitud;


        row.addEventListener('click', () => {
            this.actionClickRow(localizacion);
        });
    }

    actionClickRow(localizacion) {
        this.btnRegistrar.classList.add('disabled');
        this.btnRegistrar.disabled = true;
        this.llenarInputs(localizacion);
        this.validateForm();
    }
    //END llenar la tabla
    ...
    llenarCantones(cantones) {
        const provincia = cantones.idProvincia;
        const longLat = this.obtenerLatitudyLongitud(provincia);
        console.log(longLat);
        this.map.setCenter([longLat[0], longLat[1]]);
        this.map.setZoom(14);
    ...
    obtenerLatitudyLongitud(idProvincia) {
        let longLat;
        for (const provincia of this.objProvincias) {
            if (provincia.idProvincia === idProvincia) {
                longLat = [provincia.lng, provincia.lat];
            }
        }
        return longLat;
    }
    ...
    //Llenar los inputs cuando se seleciona uno de la tabla
    llenarInputs(localizacion) {
        this.clearForm();
        const nombre = document.getElementById('txtNombre');
        const otrasSennas = document.getElementById('txtOtrasSennas');
        const latitud = document.getElementById('txtLatitud');
        const longitud = document.getElementById('txtLongitud');

        nombre.value = localizacion.Nombre;
        otrasSennas.value = localizacion.OtrasSennas;
        latitud.value = localizacion.Latitud;
        longitud.value = localizacion.Longitud;

        this.map.setCenter([localizacion.Longitud, localizacion.Latitud]);
        this.map.setZoom(18);

        //this.fillsltProvincia();
        this.sltProvincias.value = this.obtenerProvinciaId(localizacion.Provincia);


        const cantones = this.objCantones.filter(canton => canton.idProvincia === parseInt(this.sltProvincias.value));
        this.llenarCantones(cantones);
        this.sltCantones.value = this.obtenerCantonId(localizacion.Canton, parseInt(this.sltProvincias.value));

        const distritos = this.objDistritos.filter(distrito => distrito.idCanton === parseInt(this.sltCantones.value));
        this.llenarDistritos(distritos);
        this.sltDistritos.value = this.obtenerDistritoId(localizacion.Distrito, parseInt(this.sltCantones.value));
        this.actualLocalizacion = localizacion;
    }

    obtenerProvinciaId(nombreProvincia) {
        let idProvincia;
        for (const provincia of this.objProvincias) {
    ...
    obtenerCantonId(nombreCanton, idProvincia) {
        let idCanton;
        for (const canton of this.objCantones) {
            if (canton.nombre === nombreCanton && canton.idProvincia === idProvincia) {
    ...
    obtenerDistritoId(nombreDistrito, idCanton) {
        let idDistrito;
        for (const distrito of this.objDistritos) {
            if (distrito.nombre === nombreDistrito && distrito.idCanton === idCanton) {
    ...
    desactivarBtn() {
        ...
        this.btnEliminar.classList.add('disabled');
    ...
    activarBtn() {
        ...
        this.btnEliminar.classList.remove('disabled');
        this.btnEliminar.removeAttribute('disabled');
    }
    ...
    obtenerProvinciaNombre(id) {
        let nombreProvincia;
        for (const provincia of this.objProvincias) {
            if (provincia.idProvincia === id) {
    ...
    obtenerCantonNombre(id) {
        let nombreCanton;
        for (const canton of this.objCantones) {
            if (canton.idCanton === id) {
    ...
    obtenerDistritoNombre(id) {
        let nombreDistrito;
        for (const distrito of this.objDistritos) {
            if (distrito.idDistrito === id) {
    ...
    getValuesForm() {
        ...
        const localizacionData = this.ctrlActions.GetDataForm('frmEditar');
        localizacionData.Provincia = this.obtenerProvinciaNombre(parseInt(localizacionData.Provincia));
        localizacionData.Canton = this.obtenerCantonNombre(parseInt(localizacionData.Canton));
        localizacionData.Distrito = this.obtenerDistritoNombre(parseInt(localizacionData.Distrito));
        localizacionData.IdUsuario = sessionStorage.getItem('IdUsuario');
        this.ctrlActions.PostToAPI('Localizacion', localizacionData);

        console.log(localizacionData);
    ...
    modificarLocalizacion() {
        ...
        localizacionData.Canton = this.obtenerCantonNombre(parseInt(localizacionData.Canton));
        localizacionData.Distrito = this.obtenerDistritoNombre(parseInt(localizacionData.Distrito));
        localizacionData.IdLocalizacion = this.actualLocalizacion.IdLocalizacion;
        console.log(localizacionData);

        this.ctrlActions.PutToAPI('Localizacion', localizacionData);
    ...
    {
        this.ctrlActions.DeleteToAPI('Localizacion', this.actualLocalizacion);
        $("#ready").modal('hide');
        this.clearForm();
        location.reload(true);
    }
    ...
    crearEventos()
    {
        ...
        this.btnModificar.addEventListener('click', () => {
            this.modificarLocalizacion();
            this.validateForm();
        });

        this.btnEliminar.addEventListener('click', () => { this.verificarEliminacion() });
        this.btnCancelar.addEventListener('click', () => { this.cancelarAcciones() });
        ...
        btnDelete.addEventListener('click', () => { this.eliminarLocalizacion() });

        const bntCancel = document.getElementById('btnCerrar');
        btnCancel.addEventListener('click', () => { this.cancelarEliminar() });
    }
...
document.addEventListener("DOMContentLoaded", function (event) {
    const controller = new vListarLocalizaciones();
