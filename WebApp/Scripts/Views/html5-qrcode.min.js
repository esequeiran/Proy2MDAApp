...
GridSampler = {
    checkAndNudgePoints: function (d, a) {
        for (var c = qrcode.width, b = qrcode.height, e = !0, g = 0; g < a.Length && e; g += 2) { var f = Math.floor(a[g]), h = Math.floor(a[g + 1]); if (-1 > f || f > c || -1 > h || h > b) throw "Error.checkAndNudgePoints "; e = !1; -1 == f ? (a[g] = 0, e = !0) : f == c && (a[g] = c - 1, e = !0); -1 == h ? (a[g + 1] = 0, e = !0) : h == b && (a[g + 1] = b - 1, e = !0) } e = !0; for (g = a.Length - 2; 0 <= g && e; g -= 2) {
            f = Math.floor(a[g]); h = Math.floor(a[g + 1]); if (-1 > f || f > c || -1 > h || h > b) throw "Error.checkAndNudgePoints "; e = !1; -1 == f ? (a[g] = 0, e = !0) : f == c && (a[g] = c - 1, e = !0); -1 == h ? (a[g +
                1] = 0, e = !0) : h == b && (a[g + 1] = b - 1, e = !0)
        }
    }, sampleGrid3: function (d, a, c) {
        for (var b = new BitMatrix(a), e = Array(a << 1), g = 0; g < a; g++) {
            for (var f = e.length, h = g + 0.5, j = 0; j < f; j += 2)e[j] = (j >> 1) + 0.5, e[j + 1] = h; c.transformPoints1(e); GridSampler.checkAndNudgePoints(d, e); try {
                for (j = 0; j < f; j += 2) {
                    var k = 4 * Math.floor(e[j]) + 4 * Math.floor(e[j + 1]) * qrcode.width, n = d[Math.floor(e[j]) + qrcode.width * Math.floor(e[j + 1])]; qrcode.imagedata.data[k] = n ? 255 : 0; qrcode.imagedata.data[k + 1] = n ? 255 : 0; qrcode.imagedata.data[k + 2] = 0; qrcode.imagedata.data[k + 3] =
                        255; n && b.set_Renamed(j >> 1, g)
                }
            } catch (l) { throw "Error.checkAndNudgePoints"; }
        } return b
    }, sampleGridx: function (d, a, c, b, e, g, f, h, j, k, n, l, s, r, m, q, x, y) { c = PerspectiveTransform.quadrilateralToQuadrilateral(c, b, e, g, f, h, j, k, n, l, s, r, m, q, x, y); return GridSampler.sampleGrid3(d, a, c) }
}; function ECB(d, a) { this.count = d; this.dataCodewords = a; this.__defineGetter__("Count", function () { return this.count }); this.__defineGetter__("DataCodewords", function () { return this.dataCodewords }) }
...
this.a11 = d; this.a12 = b; this.a13 = f; this.a21 = a; this.a22 = e; this.a23 = h; this.a31 = c; this.a32 = g; this.a33 = j; this.transformPoints1 = function (b) { for (var e = b.length, c = this.a11, a = this.a12, g = this.a13, d = this.a21, f = this.a22, h = this.a23, j = this.a31, z = this.a32, A = this.a33, t = 0; t < e; t += 2) { var u = b[t], v = b[t + 1], w = g * u + h * v + A; b[t] = (c * u + d * v + j) / w; b[t + 1] = (a * u + f * v + z) / w } }; this.transformPoints2 = function (b, e) {
    for (var c = b.length, a = 0; a < c; a++) {
        var g = b[a], d = e[a], f = this.a13 * g + this.a23 * d + this.a33;
        b[a] = (this.a11 * g + this.a21 * d + this.a31) / f; e[a] = (this.a12 * g + this.a22 * d + this.a32) / f
    }
}; this.buildAdjoint = function () { return new PerspectiveTransform(this.a22 * this.a33 - this.a23 * this.a32, this.a23 * this.a31 - this.a21 * this.a33, this.a21 * this.a32 - this.a22 * this.a31, this.a13 * this.a32 - this.a12 * this.a33, this.a11 * this.a33 - this.a13 * this.a31, this.a12 * this.a31 - this.a11 * this.a32, this.a12 * this.a23 - this.a13 * this.a22, this.a13 * this.a21 - this.a11 * this.a23, this.a11 * this.a22 - this.a12 * this.a21) }; this.times = function (b) {
...
function Detector(d) {
this.image = d; this.resultPointCallback = null; this.sizeOfBlackWhiteBlackRun = function (a, c, b, e) { var g = Math.abs(e - c) > Math.abs(b - a); if (g) { var d = a; a = c; c = d; d = b; b = e; e = d } for (var h = Math.abs(b - a), j = Math.abs(e - c), k = -h >> 1, n = c < e ? 1 : -1, l = a < b ? 1 : -1, s = 0, r = a, d = c; r != b; r += l) { var m = g ? d : r, q = g ? r : d; 1 == s ? this.image[m + q * qrcode.width] && s++ : this.image[m + q * qrcode.width] || s++; if (3 == s) return e = r - a, c = d - c, Math.sqrt(e * e + c * c); k += j; if (0 < k) { if (d == e) break; d += n; k -= h } } a = b - a; c = e - c; return Math.sqrt(a * a + c * c) }; this.sizeOfBlackWhiteBlackRunBothWays =
    function (a, c, b, e) { var g = this.sizeOfBlackWhiteBlackRun(a, c, b, e), d = 1; b = a - (b - a); 0 > b ? (d = a / (a - b), b = 0) : b >= qrcode.width && (d = (qrcode.width - 1 - a) / (b - a), b = qrcode.width - 1); e = Math.floor(c - (e - c) * d); d = 1; 0 > e ? (d = c / (c - e), e = 0) : e >= qrcode.height && (d = (qrcode.height - 1 - c) / (e - c), e = qrcode.height - 1); b = Math.floor(a + (b - a) * d); g += this.sizeOfBlackWhiteBlackRun(a, c, b, e); return g - 1 }; this.calculateModuleSizeOneWay = function (a, c) {
        var b = this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(a.X), Math.floor(a.Y), Math.floor(c.X), Math.floor(c.Y)),
        e = this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(c.X), Math.floor(c.Y), Math.floor(a.X), Math.floor(a.Y)); return isNaN(b) ? e / 7 : isNaN(e) ? b / 7 : (b + e) / 14
    }; this.calculateModuleSize = function (a, c, b) { return (this.calculateModuleSizeOneWay(a, c) + this.calculateModuleSizeOneWay(a, b)) / 2 }; this.distance = function (a, c) { xDiff = a.X - c.X; yDiff = a.Y - c.Y; return Math.sqrt(xDiff * xDiff + yDiff * yDiff) }; this.computeDimension = function (a, c, b, e) {
        c = Math.round(this.distance(a, c) / e); a = Math.round(this.distance(a, b) / e); a = (c + a >> 1) + 7; switch (a &
            3) { case 0: a++; break; case 2: a--; break; case 3: throw "Error"; }return a
    }; this.findAlignmentInRegion = function (a, c, b, e) { var g = Math.floor(e * a); e = Math.max(0, c - g); c = Math.min(qrcode.width - 1, c + g); if (c - e < 3 * a) throw "Error"; var d = Math.max(0, b - g); b = Math.min(qrcode.height - 1, b + g); return (new AlignmentPatternFinder(this.image, e, d, c - e, b - d, a, this.resultPointCallback)).find() }; this.createTransform = function (a, c, b, e, d) {
        d -= 3.5; var f, h, j; null != e ? (f = e.X, e = e.Y, h = j = d - 3) : (f = c.X - a.X + b.X, e = c.Y - a.Y + b.Y, h = j = d); return PerspectiveTransform.quadrilateralToQuadrilateral(3.5,
            3.5, d, 3.5, h, j, 3.5, d, a.X, a.Y, c.X, c.Y, f, e, b.X, b.Y)
    }; this.sampleGrid = function (a, c, b) { return GridSampler.sampleGrid3(a, b, c) }; this.processFinderPatternInfo = function (a) {
        var c = a.TopLeft, b = a.TopRight; a = a.BottomLeft; var e = this.calculateModuleSize(c, b, a); if (1 > e) throw "Error"; var d = this.computeDimension(c, b, a, e), f = Version.getProvisionalVersionForDimension(d), h = f.DimensionForVersion - 7, j = null; if (0 < f.AlignmentPatternCenters.length) {
            h = 1 - 3 / h; f = Math.floor(c.X + h * (b.X - c.X + a.X - c.X)); for (h = Math.floor(c.Y + h * (b.Y - c.Y + a.Y -
                c.Y)); ;) { j = this.findAlignmentInRegion(e, f, h, 4); break }
        } e = this.createTransform(c, b, a, j, d); d = this.sampleGrid(this.image, e, d); return new DetectorResult(d, null == j ? [a, c, b] : [a, c, b, j])
    }; this.detect = function () { var a = (new FinderPatternFinder).findFinderPattern(this.image); return this.processFinderPatternInfo(a) }
}
...
function BitMatrixParser(d) {
    var a = d.Dimension; if (21 > a || 1 != (a & 3)) throw "Error BitMatrixParser"; this.bitMatrix = d; this.parsedFormatInfo = this.parsedVersion = null; this.copyBit = function (c, b, e) { return this.bitMatrix.get_Renamed(c, b) ? e << 1 | 1 : e << 1 }; this.readFormatInformation = function () {
        if (null != this.parsedFormatInfo) return this.parsedFormatInfo; for (var c = 0, b = 0; 6 > b; b++)c = this.copyBit(b, 8, c); c = this.copyBit(7, 8, c); c = this.copyBit(8, 8, c); c = this.copyBit(8, 7, c); for (b = 5; 0 <= b; b--)c = this.copyBit(8, b, c); this.parsedFormatInfo =
            FormatInformation.decodeFormatInformation(c); if (null != this.parsedFormatInfo) return this.parsedFormatInfo; for (var e = this.bitMatrix.Dimension, c = 0, a = e - 8, b = e - 1; b >= a; b--)c = this.copyBit(b, 8, c); for (b = e - 7; b < e; b++)c = this.copyBit(8, b, c); this.parsedFormatInfo = FormatInformation.decodeFormatInformation(c); if (null != this.parsedFormatInfo) return this.parsedFormatInfo; throw "Error readFormatInformation";
    }; this.readVersion = function () {
        if (null != this.parsedVersion) return this.parsedVersion; var c = this.bitMatrix.Dimension,
...
this.field = d; this.decode = function (a, c) {
    for (var b = new GF256Poly(this.field, a), e = Array(c), d = 0; d < e.length; d++)e[d] = 0; for (var f = !0, d = 0; d < c; d++) { var h = b.evaluateAt(this.field.exp(d)); e[e.length - 1 - d] = h; 0 != h && (f = !1) } if (!f) {
        d = new GF256Poly(this.field, e); b = this.runEuclideanAlgorithm(this.field.buildMonomial(c, 1), d, c); d = b[1]; b = this.findErrorLocations(b[0]); e = this.findErrorMagnitudes(d, b, !1); for (d = 0; d < b.length; d++) {
            f = a.length - 1 - this.field.log(b[d]); if (0 > f) throw "ReedSolomonException Bad error location";
            a[f] = GF256.addOrSubtract(a[f], e[d])
        }
    }
}; this.runEuclideanAlgorithm = function (a, c, b) {
    if (a.Degree < c.Degree) { var e = a; a = c; c = e } for (var e = this.field.One, d = this.field.Zero, f = this.field.Zero, h = this.field.One; c.Degree >= Math.floor(b / 2);) {
        var j = a, k = e, n = f; a = c; e = d; f = h; if (a.Zero) throw "r_{i-1} was zero"; c = j; h = this.field.Zero; d = a.getCoefficient(a.Degree); for (d = this.field.inverse(d); c.Degree >= a.Degree && !c.Zero;) {
            var j = c.Degree - a.Degree, l = this.field.multiply(c.getCoefficient(c.Degree), d), h = h.addOrSubtract(this.field.buildMonomial(j,
                l)); c = c.addOrSubtract(a.multiplyByMonomial(j, l))
        } d = h.multiply1(e).addOrSubtract(k); h = h.multiply1(f).addOrSubtract(n)
    } b = h.getCoefficient(0); if (0 == b) throw "ReedSolomonException sigmaTilde(0) was zero"; b = this.field.inverse(b); a = h.multiply2(b); b = c.multiply2(b); return [a, b]
}; this.findErrorLocations = function (a) {
    var c = a.Degree; if (1 == c) return Array(a.getCoefficient(1)); for (var b = Array(c), e = 0, d = 1; 256 > d && e < c; d++)0 == a.evaluateAt(d) && (b[e] = this.field.inverse(d), e++); if (e != c) throw "Error locator degree does not match number of roots";
    return b
}; this.findErrorMagnitudes = function (a, c, b) { for (var e = c.length, d = Array(e), f = 0; f < e; f++) { for (var h = this.field.inverse(c[f]), j = 1, k = 0; k < e; k++)f != k && (j = this.field.multiply(j, GF256.addOrSubtract(1, this.field.multiply(c[k], h)))); d[f] = this.field.multiply(a.evaluateAt(h), this.field.inverse(j)); b && (d[f] = this.field.multiply(d[f], h)) } return d }
}
function GF256Poly(d, a) {
    if (null == a || 0 == a.length) throw "System.ArgumentException"; this.field = d; var c = a.length; if (1 < c && 0 == a[0]) { for (var b = 1; b < c && 0 == a[b];)b++; if (b == c) this.coefficients = d.Zero.coefficients; else { this.coefficients = Array(c - b); for (c = 0; c < this.coefficients.length; c++)this.coefficients[c] = 0; for (c = 0; c < this.coefficients.length; c++)this.coefficients[c] = a[b + c] } } else this.coefficients = a; this.__defineGetter__("Zero", function () { return 0 == this.coefficients[0] }); this.__defineGetter__("Degree", function () {
        return this.coefficients.length -
            1
    }); this.__defineGetter__("Coefficients", function () { return this.coefficients }); this.getCoefficient = function (b) { return this.coefficients[this.coefficients.length - 1 - b] }; this.evaluateAt = function (b) { if (0 == b) return this.getCoefficient(0); var c = this.coefficients.length; if (1 == b) { for (var a = b = 0; a < c; a++)b = GF256.addOrSubtract(b, this.coefficients[a]); return b } for (var d = this.coefficients[0], a = 1; a < c; a++)d = GF256.addOrSubtract(this.field.multiply(b, d), this.coefficients[a]); return d }; this.addOrSubtract = function (b) {
        if (this.field !=
            b.field) throw "GF256Polys do not have same GF256 field"; if (this.Zero) return b; if (b.Zero) return this; var a = this.coefficients; b = b.coefficients; if (a.length > b.length) { var c = a, a = b; b = c } for (var c = Array(b.length), h = b.length - a.length, j = 0; j < h; j++)c[j] = b[j]; for (j = h; j < b.length; j++)c[j] = GF256.addOrSubtract(a[j - h], b[j]); return new GF256Poly(d, c)
    }; this.multiply1 = function (b) {
        if (this.field != b.field) throw "GF256Polys do not have same GF256 field"; if (this.Zero || b.Zero) return this.field.Zero; var a = this.coefficients, c =
            a.length; b = b.coefficients; for (var d = b.length, j = Array(c + d - 1), k = 0; k < c; k++)for (var n = a[k], l = 0; l < d; l++)j[k + l] = GF256.addOrSubtract(j[k + l], this.field.multiply(n, b[l])); return new GF256Poly(this.field, j)
    }; this.multiply2 = function (b) { if (0 == b) return this.field.Zero; if (1 == b) return this; for (var a = this.coefficients.length, c = Array(a), d = 0; d < a; d++)c[d] = this.field.multiply(this.coefficients[d], b); return new GF256Poly(this.field, c) }; this.multiplyByMonomial = function (b, a) {
        if (0 > b) throw "System.ArgumentException"; if (0 ==
            a) return this.field.Zero; for (var c = this.coefficients.length, d = Array(c + b), j = 0; j < d.length; j++)d[j] = 0; for (j = 0; j < c; j++)d[j] = this.field.multiply(this.coefficients[j], a); return new GF256Poly(this.field, d)
    }; this.divide = function (b) {
        if (this.field != b.field) throw "GF256Polys do not have same GF256 field"; if (b.Zero) throw "Divide by 0"; for (var a = this.field.Zero, c = this, d = b.getCoefficient(b.Degree), d = this.field.inverse(d); c.Degree >= b.Degree && !c.Zero;)var j = c.Degree - b.Degree, k = this.field.multiply(c.getCoefficient(c.Degree),
            d), n = b.multiplyByMonomial(j, k), j = this.field.buildMonomial(j, k), a = a.addOrSubtract(j), c = c.addOrSubtract(n); return [a, c]
    }
}
function GF256(d) {
this.expTable = Array(256); this.logTable = Array(256); for (var a = 1, c = 0; 256 > c; c++)this.expTable[c] = a, a <<= 1, 256 <= a && (a ^= d); for (c = 0; 255 > c; c++)this.logTable[this.expTable[c]] = c; d = Array(1); d[0] = 0; this.zero = new GF256Poly(this, Array(d)); d = Array(1); d[0] = 1; this.one = new GF256Poly(this, Array(d)); this.__defineGetter__("Zero", function () { return this.zero }); this.__defineGetter__("One", function () { return this.one }); this.buildMonomial = function (b, c) {
    if (0 > b) throw "System.ArgumentException"; if (0 == c) return zero;
    for (var a = Array(b + 1), d = 0; d < a.length; d++)a[d] = 0; a[0] = c; return new GF256Poly(this, a)
}; this.exp = function (b) { return this.expTable[b] }; this.log = function (b) { if (0 == b) throw "System.ArgumentException"; return this.logTable[b] }; this.inverse = function (b) { if (0 == b) throw "System.ArithmeticException"; return this.expTable[255 - this.logTable[b]] }; this.multiply = function (b, a) { return 0 == b || 0 == a ? 0 : 1 == b ? a : 1 == a ? b : this.expTable[(this.logTable[b] + this.logTable[a]) % 255] }
    ...
    imagedata: null, width: 0, height: 0, qrCodeSymbol: null, debug: !1, sizeOfDataLengthInfo: [[10, 9, 8, 8], [12, 11, 16, 10], [14, 13, 16, 12]], callback: null, decode: function (d) {
        if (0 == arguments.length) { var a = document.getElementById("qr-canvas"), c = a.getContext("2d"); qrcode.width = a.width; qrcode.height = a.height; qrcode.imagedata = c.getImageData(0, 0, qrcode.width, qrcode.height); qrcode.result = qrcode.process(c); null != qrcode.callback && qrcode.callback(qrcode.result); return qrcode.result } var b = new Image; b.onload = function () {
            var a =
                ...
                qrcode.result = "Cross domain image reading not supported in your browser! Save it to your computer then drag and drop the file!"; null != qrcode.callback && qrcode.callback(qrcode.result);
                    return
                } try { qrcode.result = qrcode.process(c) } catch (j) { console.log(j), qrcode.result = "error decoding QR Code" } null != qrcode.callback && qrcode.callback(qrcode.result)
        }; b.src = d
    }, decode_utf8: function (d) { return decodeURIComponent(escape(d)) }, process: function (d) {
        var a = (new Date).getTime(), c = qrcode.grayScaleToBitmap(qrcode.grayscale()); if (qrcode.debug) {
            for (var b = 0; b < qrcode.height; b++)for (var e = 0; e < qrcode.width; e++) {
                var g = 4 * e + 4 * b * qrcode.width; qrcode.imagedata.data[g] = 0; qrcode.imagedata.data[g + 1] = 0; qrcode.imagedata.data[g +
                    2] = c[e + b * qrcode.width] ? 255 : 0
            } d.putImageData(qrcode.imagedata, 0, 0)
        } c = (new Detector(c)).detect(); qrcode.debug && d.putImageData(qrcode.imagedata, 0, 0); c = Decoder.decode(c.bits).DataByte; d = ""; for (b = 0; b < c.length; b++)for (e = 0; e < c[b].length; e++)d += String.fromCharCode(c[b][e]); a = (new Date).getTime() - a; console.log(a); return qrcode.decode_utf8(d)
    }, getPixel: function (d, a) {
        if (qrcode.width < d) throw "point error"; if (qrcode.height < a) throw "point error"; point = 4 * d + 4 * a * qrcode.width; return p = (33 * qrcode.imagedata.data[point] +
            34 * qrcode.imagedata.data[point + 1] + 33 * qrcode.imagedata.data[point + 2]) / 100
    }, binarize: function (d) { for (var a = Array(qrcode.width * qrcode.height), c = 0; c < qrcode.height; c++)for (var b = 0; b < qrcode.width; b++) { var e = qrcode.getPixel(b, c); a[b + c * qrcode.width] = e <= d ? !0 : !1 } return a }, getMiddleBrightnessPerArea: function (d) {
        for (var a = Math.floor(qrcode.width / 4), c = Math.floor(qrcode.height / 4), b = Array(4), e = 0; 4 > e; e++) { b[e] = Array(4); for (var g = 0; 4 > g; g++)b[e][g] = [0, 0] } for (e = 0; 4 > e; e++)for (g = 0; 4 > g; g++) {
        b[g][e][0] = 255; for (var f = 0; f <
            c; f++)for (var h = 0; h < a; h++) { var j = d[a * g + h + (c * e + f) * qrcode.width]; j < b[g][e][0] && (b[g][e][0] = j); j > b[g][e][1] && (b[g][e][1] = j) }
        } d = Array(4); for (a = 0; 4 > a; a++)d[a] = Array(4); for (e = 0; 4 > e; e++)for (g = 0; 4 > g; g++)d[g][e] = Math.floor((b[g][e][0] + b[g][e][1]) / 2); return d
        ...
        for (var a = qrcode.getMiddleBrightnessPerArea(d), c = a.length, b = Math.floor(qrcode.width / c), e = Math.floor(qrcode.height / c), g = Array(qrcode.height * qrcode.width), f = 0; f < c; f++)for (var h = 0; h < c; h++)for (var j = 0; j < e; j++)for (var k = 0; k < b; k++)g[b *
            h + k + (e * f + j) * qrcode.width] = d[b * h + k + (e * f + j) * qrcode.width] < a[h][f] ? !0 : !1; return g
    }, grayscale: function () { for (var d = Array(qrcode.width * qrcode.height), a = 0; a < qrcode.height; a++)for (var c = 0; c < qrcode.width; c++) { var b = qrcode.getPixel(c, a); d[c + a * qrcode.width] = b } return d }
}; function URShift(d, a) { return 0 <= d ? d >> a : (d >> a) + (2 << ~a) } Array.prototype.remove = function (d, a) { var c = this.slice((a || d) + 1 || this.length); this.length = 0 > d ? this.length + d : d; return this.push.apply(this, c) };
...
this.x = d; this.y = a; this.count = 1; this.estimatedModuleSize = c; this.__defineGetter__("EstimatedModuleSize", function () { return this.estimatedModuleSize }); this.__defineGetter__("Count", function () { return this.count }); this.__defineGetter__("X", function () { return this.x }); this.__defineGetter__("Y", function () { return this.y }); this.incrementCount = function () { this.count++ }; this.aboutEquals = function (b, a, c) {
    return Math.abs(a - this.y) <= b && Math.abs(c - this.x) <= b ? (b = Math.abs(b - this.estimatedModuleSize),
        1 >= b || 1 >= b / this.estimatedModuleSize) : !1
}
} function FinderPatternInfo(d) { this.bottomLeft = d[0]; this.topLeft = d[1]; this.topRight = d[2]; this.__defineGetter__("BottomLeft", function () { return this.bottomLeft }); this.__defineGetter__("TopLeft", function () { return this.topLeft }); this.__defineGetter__("TopRight", function () { return this.topRight }) }
function FinderPatternFinder() {
this.image = null; this.possibleCenters = []; this.hasSkipped = !1; this.crossCheckStateCount = [0, 0, 0, 0, 0]; this.resultPointCallback = null; this.__defineGetter__("CrossCheckStateCount", function () { this.crossCheckStateCount[0] = 0; this.crossCheckStateCount[1] = 0; this.crossCheckStateCount[2] = 0; this.crossCheckStateCount[3] = 0; this.crossCheckStateCount[4] = 0; return this.crossCheckStateCount }); this.foundPatternCross = function (d) {
    for (var a = 0, c = 0; 5 > c; c++) { var b = d[c]; if (0 == b) return !1; a += b } if (7 >
        a) return !1; a = Math.floor((a << INTEGER_MATH_SHIFT) / 7); c = Math.floor(a / 2); return Math.abs(a - (d[0] << INTEGER_MATH_SHIFT)) < c && Math.abs(a - (d[1] << INTEGER_MATH_SHIFT)) < c && Math.abs(3 * a - (d[2] << INTEGER_MATH_SHIFT)) < 3 * c && Math.abs(a - (d[3] << INTEGER_MATH_SHIFT)) < c && Math.abs(a - (d[4] << INTEGER_MATH_SHIFT)) < c
}; this.centerFromEnd = function (d, a) { return a - d[4] - d[3] - d[2] / 2 }; this.crossCheckVertical = function (d, a, c, b) {
    for (var e = this.image, g = qrcode.height, f = this.CrossCheckStateCount, h = d; 0 <= h && e[a + h * qrcode.width];)f[2]++, h--; if (0 >
        h) return NaN; for (; 0 <= h && !e[a + h * qrcode.width] && f[1] <= c;)f[1]++, h--; if (0 > h || f[1] > c) return NaN; for (; 0 <= h && e[a + h * qrcode.width] && f[0] <= c;)f[0]++, h--; if (f[0] > c) return NaN; for (h = d + 1; h < g && e[a + h * qrcode.width];)f[2]++, h++; if (h == g) return NaN; for (; h < g && !e[a + h * qrcode.width] && f[3] < c;)f[3]++, h++; if (h == g || f[3] >= c) return NaN; for (; h < g && e[a + h * qrcode.width] && f[4] < c;)f[4]++, h++; return f[4] >= c || 5 * Math.abs(f[0] + f[1] + f[2] + f[3] + f[4] - b) >= 2 * b ? NaN : this.foundPatternCross(f) ? this.centerFromEnd(f, h) : NaN
}; this.crossCheckHorizontal =
    function (d, a, c, b) {
        for (var e = this.image, g = qrcode.width, f = this.CrossCheckStateCount, h = d; 0 <= h && e[h + a * qrcode.width];)f[2]++, h--; if (0 > h) return NaN; for (; 0 <= h && !e[h + a * qrcode.width] && f[1] <= c;)f[1]++, h--; if (0 > h || f[1] > c) return NaN; for (; 0 <= h && e[h + a * qrcode.width] && f[0] <= c;)f[0]++, h--; if (f[0] > c) return NaN; for (h = d + 1; h < g && e[h + a * qrcode.width];)f[2]++, h++; if (h == g) return NaN; for (; h < g && !e[h + a * qrcode.width] && f[3] < c;)f[3]++, h++; if (h == g || f[3] >= c) return NaN; for (; h < g && e[h + a * qrcode.width] && f[4] < c;)f[4]++, h++; return f[4] >=
        ...
        var b = d[0] + d[1] + d[2] + d[3] + d[4]; c = this.centerFromEnd(d, c); a = this.crossCheckVertical(a, Math.floor(c), d[2], b); if (!isNaN(a) && (c = this.crossCheckHorizontal(Math.floor(c), Math.floor(a), d[2], b), !isNaN(c))) {
            d = b / 7; for (var b = !1, e = this.possibleCenters.length, g = 0; g < e; g++) { var f = this.possibleCenters[g]; if (f.aboutEquals(d, a, c)) { f.incrementCount(); b = !0; break } } b || (c = new FinderPattern(c,
        ...
        var d = this.possibleCenters.length; if (3 > d) throw "Couldn't find enough finder patterns"; if (3 < d) {
            for (var a = 0, c = 0; c < d; c++)a += this.possibleCenters[c].EstimatedModuleSize; d = a / d; for (c = 0; c < this.possibleCenters.length && 3 < this.possibleCenters.length; c++)Math.abs(this.possibleCenters[c].EstimatedModuleSize - d) > 0.2 * d && (this.possibleCenters.remove(c),
                c--)
        } return [this.possibleCenters[0], this.possibleCenters[1], this.possibleCenters[2]]
    }; this.findRowSkip = function () { var d = this.possibleCenters.length; if (1 >= d) return 0; for (var a = null, c = 0; c < d; c++) { var b = this.possibleCenters[c]; if (b.Count >= CENTER_QUORUM) if (null == a) a = b; else return this.hasSkipped = !0, Math.floor((Math.abs(a.X - b.X) - Math.abs(a.Y - b.Y)) / 2) } return 0 }; this.haveMultiplyConfirmedCenters = function () {
        for (var d = 0, a = 0, c = this.possibleCenters.length, b = 0; b < c; b++) {
            var e = this.possibleCenters[b]; e.Count >= CENTER_QUORUM &&
                (d++, a += e.EstimatedModuleSize)
        } if (3 > d) return !1; for (var d = a / c, g = 0, b = 0; b < c; b++)e = this.possibleCenters[b], g += Math.abs(e.EstimatedModuleSize - d); return g <= 0.05 * a
    }; this.findFinderPattern = function (d) {
    this.image = d; var a = qrcode.height, c = qrcode.width, b = Math.floor(3 * a / (4 * MAX_MODULES)); b < MIN_SKIP && (b = MIN_SKIP); for (var e = !1, g = Array(5), f = b - 1; f < a && !e; f += b) {
    g[0] = 0; g[1] = 0; g[2] = 0; g[3] = 0; for (var h = g[4] = 0, j = 0; j < c; j++)if (d[j + f * qrcode.width]) 1 == (h & 1) && h++, g[h]++; else if (0 == (h & 1)) if (4 == h) if (this.foundPatternCross(g)) {
        if (h =
            this.handlePossibleCenter(g, f, j)) b = 2, this.hasSkipped ? e = this.haveMultiplyConfirmedCenters() : (h = this.findRowSkip(), h > g[2] && (f += h - g[2] - b, j = c - 1)); else { do j++; while (j < c && !d[j + f * qrcode.width]); j-- } h = 0; g[0] = 0; g[1] = 0; g[2] = 0; g[3] = 0; g[4] = 0
    } else g[0] = g[2], g[1] = g[3], g[2] = g[4], g[3] = 1, g[4] = 0, h = 3; else g[++h]++; else g[h]++; this.foundPatternCross(g) && this.handlePossibleCenter(g, f, c) && (b = g[0], this.hasSkipped && (e = haveMultiplyConfirmedCenters()))
    } d = this.selectBestPatterns(); qrcode.orderBestPatterns(d); return new FinderPatternInfo(d)
    }
}
function AlignmentPattern(d, a, c) {
this.x = d; this.y = a; this.count = 1; this.estimatedModuleSize = c; this.__defineGetter__("EstimatedModuleSize", function () { return this.estimatedModuleSize }); this.__defineGetter__("Count", function () { return this.count }); this.__defineGetter__("X", function () { return Math.floor(this.x) }); this.__defineGetter__("Y", function () { return Math.floor(this.y) }); this.incrementCount = function () { this.count++ }; this.aboutEquals = function (b, a, c) {
...
this.image = d; this.possibleCenters = []; this.startX = a; this.startY = c; this.width = b; this.height = e; this.moduleSize = g; this.crossCheckStateCount = [0, 0, 0]; this.resultPointCallback = f; this.centerFromEnd = function (b, a) { return a - b[2] - b[1] / 2 }; this.foundPatternCross = function (b) { for (var a = this.moduleSize, c = a / 2, d = 0; 3 > d; d++)if (Math.abs(a - b[d]) >= c) return !1; return !0 }; this.crossCheckVertical = function (b, a, c, d) {
    var e = this.image, g = qrcode.height, f = this.crossCheckStateCount; f[0] =
        0; f[1] = 0; f[2] = 0; for (var m = b; 0 <= m && e[a + m * qrcode.width] && f[1] <= c;)f[1]++, m--; if (0 > m || f[1] > c) return NaN; for (; 0 <= m && !e[a + m * qrcode.width] && f[0] <= c;)f[0]++, m--; if (f[0] > c) return NaN; for (m = b + 1; m < g && e[a + m * qrcode.width] && f[1] <= c;)f[1]++, m++; if (m == g || f[1] > c) return NaN; for (; m < g && !e[a + m * qrcode.width] && f[2] <= c;)f[2]++, m++; return f[2] > c || 5 * Math.abs(f[0] + f[1] + f[2] - d) >= 2 * d ? NaN : this.foundPatternCross(f) ? this.centerFromEnd(f, m) : NaN
}; this.handlePossibleCenter = function (b, a, c) {
    var d = b[0] + b[1] + b[2]; c = this.centerFromEnd(b,
        c); a = this.crossCheckVertical(a, Math.floor(c), 2 * b[1], d); if (!isNaN(a)) { b = (b[0] + b[1] + b[2]) / 3; for (var d = this.possibleCenters.length, e = 0; e < d; e++)if (this.possibleCenters[e].aboutEquals(b, a, c)) return new AlignmentPattern(c, a, b); c = new AlignmentPattern(c, a, b); this.possibleCenters.push(c); null != this.resultPointCallback && this.resultPointCallback.foundPossibleResultPoint(c) } return null
}; this.find = function () {
    for (var a = this.startX, e = this.height, g = a + b, f = c + (e >> 1), l = [0, 0, 0], s = 0; s < e; s++) {
        var r = f + (0 == (s & 1) ? s + 1 >> 1 : -(s +
            1 >> 1)); l[0] = 0; l[1] = 0; l[2] = 0; for (var m = a; m < g && !d[m + qrcode.width * r];)m++; for (var q = 0; m < g;) { if (d[m + r * qrcode.width]) if (1 == q) l[q]++; else if (2 == q) { if (this.foundPatternCross(l) && (q = this.handlePossibleCenter(l, r, m), null != q)) return q; l[0] = l[2]; l[1] = 1; l[2] = 0; q = 1 } else l[++q]++; else 1 == q && q++, l[q]++; m++ } if (this.foundPatternCross(l) && (q = this.handlePossibleCenter(l, r, g), null != q)) return q
...
this.blockPointer = 0; this.bitPointer = 7; this.dataLength = 0; this.blocks = d; this.numErrorCorrectionCode = c; 9 >= a ? this.dataLengthMode = 0 : 10 <= a && 26 >= a ? this.dataLengthMode = 1 : 27 <= a && 40 >= a && (this.dataLengthMode = 2); this.getNextBits = function (b) {
    var a = 0; if (b < this.bitPointer + 1) { for (var c = 0, a = 0; a < b; a++)c += 1 << a; c <<= this.bitPointer - b + 1; a = (this.blocks[this.blockPointer] & c) >> this.bitPointer - b + 1; this.bitPointer -= b; return a } if (b < this.bitPointer + 1 + 8) {
        for (var d = 0, a = 0; a < this.bitPointer +
            1; a++)d += 1 << a; a = (this.blocks[this.blockPointer] & d) << b - (this.bitPointer + 1); this.blockPointer++; a += this.blocks[this.blockPointer] >> 8 - (b - (this.bitPointer + 1)); this.bitPointer -= b % 8; 0 > this.bitPointer && (this.bitPointer = 8 + this.bitPointer); return a
    } if (b < this.bitPointer + 1 + 16) {
        for (a = c = d = 0; a < this.bitPointer + 1; a++)d += 1 << a; d = (this.blocks[this.blockPointer] & d) << b - (this.bitPointer + 1); this.blockPointer++; var h = this.blocks[this.blockPointer] << b - (this.bitPointer + 1 + 8); this.blockPointer++; for (a = 0; a < b - (this.bitPointer +
            1 + 8); a++)c += 1 << a; c <<= 8 - (b - (this.bitPointer + 1 + 8)); a = d + h + ((this.blocks[this.blockPointer] & c) >> 8 - (b - (this.bitPointer + 1 + 8))); this.bitPointer -= (b - 8) % 8; 0 > this.bitPointer && (this.bitPointer = 8 + this.bitPointer); return a
    } return 0
}; this.NextMode = function () { return this.blockPointer > this.blocks.length - this.numErrorCorrectionCode - 2 ? 0 : this.getNextBits(4) }; this.getDataLength = function (b) { for (var a = 0; 1 != b >> a;)a++; return this.getNextBits(qrcode.sizeOfDataLengthInfo[this.dataLengthMode][a]) }; this.getRomanAndFigureString =
    function (b) { var a = 0, c = "", d = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:".split(""); do if (1 < b) { var a = this.getNextBits(11), h = a % 45, c = c + d[Math.floor(a / 45)], c = c + d[h]; b -= 2 } else 1 == b && (a = this.getNextBits(6), c += d[a], b -= 1); while (0 < b); return c }; this.getFigureString = function (b) { var a = 0, c = ""; do 3 <= b ? (a = this.getNextBits(10), 100 > a && (c += "0"), 10 > a && (c += "0"), b -= 3) : 2 == b ? (a = this.getNextBits(7), 10 > a && (c += "0"), b -= 2) : 1 == b && (a = this.getNextBits(4), b -= 1), c += a; while (0 < b); return c }; this.get8bitByteArray = function (b) {
        var a =
            0, c = []; do a = this.getNextBits(8), c.push(a), b--; while (0 < b); return c
    }; this.getKanjiString = function (b) { var a = 0, c = ""; do { var a = getNextBits(13), a = (a / 192 << 8) + a % 192, d = 0, d = 40956 >= a + 33088 ? a + 33088 : a + 49472, c = c + String.fromCharCode(d); b-- } while (0 < b); return c }; this.__defineGetter__("DataByte", function () {
        var b = []; do {
            var a = this.NextMode(); if (0 == a) if (0 < b.length) break; else throw "Empty data block"; if (1 != a && 2 != a && 4 != a && 8 != a) throw "Invalid mode: " + a + " in (block:" + this.blockPointer + " bit:" + this.bitPointer + ")"; dataLength =
    ...
    d.fn.html5_qrcode = function (a, c, b) {
        var e = this.height(), g = this.width(); null == e && (e = 250); null == g && (g = 300); var f = '<canvas id="qr-canvas" width="' + (g - 2) + 'px" height="' + (e - 2) + 'px" style="display:none;"></canvas>'; this.append('<video id="html5_qrcode_video" width="' + g + 'px" height="' + e + 'px"></video>'); this.append(f); var h = d("#html5_qrcode_video").get(0), j; d("#qr-canvas").each(function (a, b) { j = b.getContext("2d") }); var k = function () {
            if (localMediaStream) {
                j.drawImage(h, 0, 0, 307, 250); try { qrcode.decode() } catch (a) { c(a) } setTimeout(k,
                    500)
            } else setTimeout(svan, 500)
        }; window.URL = window.URL || window.webkitURL || window.mozURL || window.msURL; navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia; e = function (a) { h.src = window.URL && window.URL.createObjectURL(a) || a; localMediaStream = a; h.play(); setTimeout(k, 1E3) }; navigator.getUserMedia ? navigator.getUserMedia({ video: !0 }, e, b) : console.log("Native web camera streaming (getUserMedia) not supported in this browser."); qrcode.callback =
            a
    }
})(jQuery);