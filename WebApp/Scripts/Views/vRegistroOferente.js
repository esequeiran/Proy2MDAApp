class vRegistrarOferentes {
    constructor() {
        ...
        this.sltPaises = document.getElementById('selectPaisNacimiento');
        this.btnRegistrar = document.getElementById('btnCrearUsuario');
    ...
    obtenerTipoId = function (id) {
        let tipo;
        var tiposId = new vRegistrarOferentes;
        for (const tipoId of tiposId.tiposId) {
            if (tipoId.id == id) {
                tipo = tipoId.nombre;
            }
        }
    ...
    obtenerGeneroId = function (id) {
        let genero;
        var generosId = new vRegistrarOferentes;
    ...
    obtenerPais = function (id) {
        let pais;
        var paisesId = new vRegistrarOferentes;

        for (const p of paisesId.paises) {
            if (p.idPais == id) {
                pais = p.nombre;
            }
        }
...
function vRegistroOferente() {
    ...
    this.tblOferenteId = 'tblOferente';
    this.service = 'Oferente';
    this.ctrlActions = new ControlActions();
    this.columns = "Nombre,Apellido1,Apellido2,Cedula,TipoCedula,Genero,Correo,Telefono,PaisNacimiento,FecNacimiento,NombreComercial,RazonSocial,CedulaJuridica,FecCreacion,Descripcion,TipoCedulaComercial";




    this.RetrieveAll = function () {
        this.ctrlActions.FillTable(this.service, this.tblOferenteId, false);
    }

    this.ReloadTable = function () {
        this.ctrlActions.FillTable(this.service, this.tblOferenteId, true);
    }
    ...
    this.CrearUsuario = function () {
        ...
        var fecCreacion = $('#txtFecCreacion')[0].value.split('-');

        var fecNacimiento = $('#txtFecNacimiento')[0].value.split('-');
        var edad = (fecCreacion[0] - fecNacimiento[0]);
        if (edad < 18) {
            this.ctrlActions = new ControlActions();
            this.ctrlActions.ShowMessage('E', "Debe ser mayor a 18 años para poder registrarse en la aplicación.");
            return false;
        }
        var desc = $('#txtDescripcion')[0].value;
        if (desc == "") {
            this.ctrlActions = new ControlActions();
            this.ctrlActions.ShowMessage('E', "Debe de poner una descripción para la empresa.");
            return false;
        }

        var nombreListas = new vRegistrarOferentes;
        var oferenteData = {};
        oferenteData = this.ctrlActions.GetDataForm('frmUsuario');


        oferenteData.TipoCedula = nombreListas.obtenerTipoId(oferenteData.TipoCedula);
        oferenteData.Genero = nombreListas.obtenerGeneroId(oferenteData.Genero);
        oferenteData.PaisNacimiento = nombreListas.obtenerPais(oferenteData.PaisNacimiento);
        ...
        this.ctrlActions.PostToAPI(this.service, oferenteData, function (success) {
            if (success) {
                ...
                $('#RegistroDocumento').css('display', 'block');
                sessionStorage.setItem('CedulaRegistro', $('#txtCedula').val());
...
$(document).ready(function () {
    ...
    var vregistroOferente = new vRegistroOferente();
    vregistroOferente.RetrieveAll();
});

document.addEventListener("DOMContentLoaded", function (event) {
    const controller = new vRegistrarOferentes();
